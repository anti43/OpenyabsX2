...
<html>
    ...
    <body>
        ...
        <div id="list-deletedEntry" class="content scaffold-list" role="main">
            <h1><g:message code="default.list.label" args="[entityName]" /></h1>
            <g:if test="${flash.message}">
                <div class="alert alert-info" role="status">${flash.message}</div>
            </g:if>
            <div class="yabs-data container">
                <dataTable:table id="deletedEntryData"
                                 controller="${params.controller}"
                                 serverURL="${createLink(controller: 'deletedEntry', action: 'indexData')}"
                                 config="${tableConfig}"/>
            </div>
        </div>
    </body>
